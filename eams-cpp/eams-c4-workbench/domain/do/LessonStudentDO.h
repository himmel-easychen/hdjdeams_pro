...
class LessonStudentDO : public BaseDO
{
	...
	//课程id
	MYSQL_SYNTHESIZE(uint64_t, lesson_id, Lesson_id);
	//班级id
	MYSQL_SYNTHESIZE(uint64_t, class_id, Class_id);
	//学员id
	MYSQL_SYNTHESIZE(uint64_t, student_id, Student_id);
	//实扣课次
	MYSQL_SYNTHESIZE(int, dec_lesson_count, dec_lesson_count);
	//应扣课次
	MYSQL_SYNTHESIZE(int, lesson_count, lesson_count);
	//点名的老师
	MYSQL_SYNTHESIZE(uint64_t, teacher_id, Teacher_id);
	//签到时间
	MYSQL_SYNTHESIZE(string, sign_time, Sign_time);
	//签到方式
	MYSQL_SYNTHESIZE(string, sign_type, Sign_type);
	//签到结果
	MYSQL_SYNTHESIZE(int, sign_state, Sign_state);
	//老师评分
	...
	//实际消费课程
	MYSQL_SYNTHESIZE(uint64_t, consume_course_id, Consume_course_id);
	//消费课程与学生关联表ID
	MYSQL_SYNTHESIZE(uint64_t, consume_student_course_id, Consume_student_course_id);
	//学生的顾问
	MYSQL_SYNTHESIZE(string, counselor,Counselor);
	//加入时间
	MYSQL_SYNTHESIZE(string, add_time, Add_time);
	//上课老师所属组织
	MYSQL_SYNTHESIZE(uint64_t, org_id, Org_id);

public:
	LessonStudentDO() : BaseDO("LessonStudentDO")
	{
		MYSQL_ADD_FIELD_PK("id", "bi", id);
		MYSQL_ADD_FIELD("lesson_id", "bi", lesson_id);
		MYSQL_ADD_FIELD("class_id", "bi", class_id);
		...
		MYSQL_ADD_FIELD("teacher_id", "bi", teacher_id);
		...
		MYSQL_ADD_FIELD("consume_course_id", "bi", consume_course_id);
		MYSQL_ADD_FIELD("consume_student_course_id", "bi", consume_student_course_id);
...
/*
* 点名记录视图对象，关联学生表的学生姓名字段
*/
class RollcallrecordViewDO : public LessonStudentDO
{
	//学生姓名
	MYSQL_SYNTHESIZE(string, studentname, StudentName);
};
typedef std::shared_ptr<RollcallrecordViewDO> PtrRollcallrecordViewDO;
