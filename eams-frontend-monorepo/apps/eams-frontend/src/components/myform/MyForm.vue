	...
	<section class="bar-container">
		...
		<section class="bar-header">
			...
			<section class="header-slot">
				...
				<el-button v-if="showback" type="info" size="small" icon="IconBack" @click="handleBack()">返回</el-button>
			</section>
		</section>
		<!-- 表单 -->
		...
		<section class="bar-footer">
			...
			<section>
				...
				<el-button v-if="!props.formattr?.disabled" type="success" icon="IconFinished" @click="submitForm">
					{{ props.submittext }}
				</el-button>
			</section>
		</section>
 ...
 */
function handleBack() {
	if (props.disableBeforeClose || props.formattr?.disabled || isEqualWithEmpty(localmodel, props.model)) {
		router.go(-1);
	} else {
