<template>
	<section class="root-container">
		<!-- 简单表格演示 -->
		<my-table tabtitle="简单表格" :tabdatacolumns="tabdatacolumns" :tabdata="tabdata" @page-change="handlePageChange" />
		<!-- 序号表格演示 -->
		<my-table
			tabtitle="序号表格"
			:istabseq="true"
		...
		>
			...
			<template #customercell="{ prop, row }">
				...
				<template v-if="prop === 'name'">
					<el-link type="primary" @click="handleRowDbclick(row)">{{ row[prop] }}-({{ row.id }})</el-link>
				</template>
				<!-- 对标签列特殊显示处理 -->
		...
		>
			...
			<template #header>
				...
				<el-button type="primary" icon="IconPlus" @click="formDialogAdd.openDialog(true)">新增</el-button>
			</template>
			<!-- 表格底部插槽 -->
